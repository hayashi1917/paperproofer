...
論文 PDF と TeX ソースをアップロードすると、Gemini が体裁上の問題点をチェックリストに沿って洗い出す論文校正アプリです。

## 開発した背景

論文執筆では、研究内容そのものだけでなく、表記ゆれ、図表の扱い、参考文献の書式、論文調の崩れといった体裁面の確認にも多くの時間がかかります。特に PDF を目視で見ながら細かな違反を探す作業は負荷が高く、見落としも起こりやすいです。

このアプリは、論文 PDF と TeX ソースを入力することで、定義済みのチェックリストに沿って形式上の問題を抽出し、修正候補と理由を一覧で確認できるようにすることを目的に開発しました。
...
- 論文 PDF と TeX ソースをアップロードして校正を開始
- Gemini に PDF と TeX ソースを渡し、チェックリストに基づく指摘事項を抽出
- 指摘ごとに問題箇所、修正候補、該当チェック項目、理由を表示
- 不要な指摘を無視リストに追加
- 修正後の PDF と TeX ソースを再アップロードして再校正
- ラウンド番号を保持しながら継続的に確認
...
- PDF と TeX ソースの両方を入力にした点
  完成版 PDF の表示結果と LaTeX ソースの記述をあわせて確認できるため、参照、数式環境、コマンドの使い方などソース側で判断しやすい問題も検出しやすくなります。
- 指摘を構造化データで返す点
...
sequenceDiagram
    ...
    U->>F: PDF と TeX をアップロード
    U->>F: 校正開始
    F->>B: /api/proofread
    B->>G: PDF と TeX を渡して解析
    G-->>B: 指摘一覧を返却
    B-->>F: issues, round_number
    U->>F: 無視または修正後PDF/TeXで再校正
```
...
| メソッド | エンドポイント | 概要 |
| --- | --- | --- |
| `POST` | `/api/proofread/` | 論文 PDF と TeX ソースをもとに指摘事項一覧を返す |
